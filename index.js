...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ espere 2minutos por favor enquanto isso beba agua! ⌛',
				success: '✔️ Sucesso ✔️',
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite DANTE|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
